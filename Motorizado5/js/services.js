...
.factory('userService', ['$http', 'localStorageService', function($http, localStorageService) {
    ...
    function checkIfLoggedIn() {
      console.log(localStorageService.get('Motorizado-Token'));
        if(localStorageService.get('Motorizado-Token'))
            return true;
        else
            return false;
    ...
    function signup(name, email, password, apellidos, dni, razon_social, ruc, telefono, tipo_usuario ,onSuccess, onError) {
        ...
        then(function(response) {
            ...
            //localStorageService.set('Motorizado-Token', response.data.token);
            onSuccess(response);
    ...
    function logout(){
        ...
        localStorageService.remove('Motorizado-Token');

    }

    function getCurrentToken(){
        return localStorageService.get('Motorizado-Token');
    }
